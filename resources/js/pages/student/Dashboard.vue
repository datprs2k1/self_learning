<template>
  <div style="border-top: 4px solid #f58635">
    ...
    <b-modal id="modal-danhsach" size="lg" :title="'Điểm số'" ref="modalDanhSach">
    ...
    nodeSelect(node, isSelected) {
      switch (node.data.type) {
        case "danhsach":
          ...
          break;
        case "diemso":
          this.getResult(node.data.subject_id, node.data.class_id).then((data) => {
            const result = data.filter(
              (item) =>
                item.question.subject_id == node.data.subject_id &&
                item.question.class_id == node.data.class_id
            );
            this.result = result;
        ...
        case "video":
          if (
            this.getVideoPath(node.data.subject_id, node.data.class_id, node.data.week) !=
              null &&
            isSelected
          ) {
