     ...
     <div id="header">
        ...
        <div class="container-fluid py-3 navbar-login">
            <div class="row w-100">
                <div class="col-md-8">
                    <div class="site-logo">
                        <a href="/"><img src="images/utt_selflearning.png" width="320px" alt="Image" class="img-fluid" /></a>
                    </div>
                </div>
                    ...
                    <div class="row h-100">
                        <div class="col-md-9 p-0">
                            <b-dropdown id="dropdown-in-header" variant="primary">
                                ...
                                <b-dropdown-item-button>Some other
        ...
        <header class="site-navbar site-navbar-target" role="banner" style="background-color: #3a454b; z-index: 1">
                    ...
                    <div class="mr-auto">
                        <nav class="site-navigation position-relative text-right" role="navigation">
                            <ul class="site-menu main-menu js-clone-nav mr-auto d-none d-lg-block p-0">
                                <li class="active">
                                    <a href="/" class="nav-link text-left home p-0 px-3 text-white"><span class="icon icon-home2"></span></a>
                                </li>
                                <li class="has-children">
                                    <a to="/about" class="nav-link text-left p-0 px-3 text-white"><span>Giới thiệu</span></a>
                                    <ul class="dropdown bg-secondary">
                                        <li>
                                            <a href="/about" class="nav-link text-left p-0 px-3 text-white"><span>Our Teacher</span></a>
                                        </li>
                                        <li>
                                            <a href="/about" class="nav-link text-left p-0 px-3 text-white"><span>Our Student</span></a>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="" class="nav-link text-left p-0 px-3 text-white"><span>Tin tức</span></a>
                                </li>
                                <li>
                                    <a href="/courses" class="nav-link text-left p-0 px-3 text-white"><span>Khóa học</span></a>
                                </li>
                                <li>
                                    <a href="/contact" class="nav-link text-left p-0 px-3 text-white"><span>Liên hệ</span></a>
                                </li>
    ...
    <div class="container-fluid" style="height: 1500px; width: 100%; margin-top: 48px">
        <div class="row h-100" style="padding-top: 24px">
            <div class="col-md-3 h-50">
                ...
                <div class="w-100 mt-3 mb-3" style="
                        ...
                        ">
                    <div class="px-3 pb-1">
                        <b-icon icon="diagram3-fill" style="
                                    color: #f58635;
                                    width: 24px;
                                    height: 24px;
                                "></b-icon>
                    ...
                    <div class="left-sidebar px-3" style="border-top: 2px solid #dddddd">
                        <div class="row">
                            ...
                            <div class="col-md-12 p-0">
                                <b-tree-view :data="treeData" :contextMenu="false" :contextMenuItems="contextMenuItems" @nodeSelect="nodeSelect" :renameNodeOnDblClick="false"></b-tree-view>
                            </div>
                        </div>
                    </div>
                </div>
            <div class="w-100 mt-3 mb-3" style="
                        ...
                        ">
                    <div class="px-3 pb-1">
                        <b-icon icon="volume-up-fill" style="
                                    color: #f58635;
                                    width: 24px;
                                    height: 24px;
                                "></b-icon>
            ...
            <div class="col-md-6 h-50 rounded" style="border-top: 3px solid rgb(245, 134, 53);
    box-shadow: rgb(0 0 0 / 5%) 0px 2px 2px 2px;padding-top: 12px;
    margin-top: 35px;">
                <div v-if="showQuestions">
                    <div v-for="(item, index) in questions" :key="item.id">
                        <h6>Câu {{index+1}}: {{item.question}}</h6>
                        <div class="form-check">
                        <input class="form-check-input" @change="setSelected" type="radio" v-model="selected[index]" :id="`exampleRadios1${index}`" :value="{ question_id: item.id, value: 1 }">
                        <label class="form-check-label" @change="setSelected" :for="`exampleRadios1${index}`">
                            {{ item.answer_A }}
                        </label>
                        </div>
                        <div class="form-check">
                        <input class="form-check-input" @change="setSelected" type="radio" v-model="selected[index]" :id="`exampleRadios2${index}`" :value="{ question_id: item.id, value: 2 }">
                        <label class="form-check-label" @change="setSelected" :for="`exampleRadios2${index}`">
                            {{ item.answer_B }}
                        </label>
                        </div>
                        <div class="form-check">
                        <input class="form-check-input" @change="setSelected" type="radio" v-model="selected[index]" :id="`exampleRadios3${index}`" :value="{ question_id: item.id, value: 3 }">
                        <label class="form-check-label" @change="setSelected" :for="`exampleRadios3${index}`">
                            {{ item.answer_C }}
                        </label>
                        </div>
                        <div class="form-check">
                        <input class="form-check-input" @change="setSelected" type="radio" v-model="selected[index]" :id="`exampleRadios4${index}`" :value="{ question_id: item.id, value: 4 }">
                        <label class="form-check-label" @change="setSelected" :for="`exampleRadios4${index}`">
                            {{ item.answer_D }}
                        </label>
                        </div>
                    </div>
            ...
            <div class="col-md-3 h-50">
                <div style="border-top: 3px solid rgb(245, 134, 53);
            box-shadow: rgb(0 0 0 / 5%) 0px 2px 2px 2px;padding-top: 12px;
...
export default {
    ...
    methods: {
            ...
            switch (node.data.type) {
                ...
                case 'slide':
                    if (this.getSlidePath(node.data.subject_id, node.data.class_id, node.data.week) != '' && isSelected) {
                        window.open(`/files/${this.getSlidePath(node.data.subject_id, node.data.class_id, node.data.week)}`, '_blank');
                ...
                case 'video':
                    if (this.getVideoPath(node.data.subject_id, node.data.class_id, node.data.week) != '' && isSelected) {
                        ...
                        this.subject_name = this.lessons.filter(lesson => lesson.subject_id == node.data.subject_id && lesson.class_id == node.data.class_id && lesson.week == node.data.week)[0].name;
                        this.showModalVideo();
        ...
        getSlidePath(subject_id, class_id, week) {
            try {
                return this.lessons.filter(lesson => lesson.subject_id == subject_id && lesson.class_id == class_id && lesson.week == week)[0].path;
            } catch (error) {
                return '';
            }
        },
        getVideoPath(subject_id, class_id, week) {
            try {
                return this.lessons.filter(lesson => lesson.subject_id == subject_id && lesson.class_id == class_id && lesson.week == week)[0].video_path;
            } catch (error) {
                return '';
            }
        },
        ...
        getQuestions(subject_id, class_id, week) {
            try {
                this.questions = this.myCourse.question.filter(question => question.subject_id == subject_id && question.class_id == class_id && question.week == week).sort(() => Math.random() - 0.5);
        ...
        submit() {
            this.$swal({
                ...
                text: (this.questions.length - this.selected.length) > 0 ? `Có ${this.questions.length - this.selected.length} câu chưa được chọn` : '',
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    ...
                    this.$store.dispatch('student/submitTest', {
