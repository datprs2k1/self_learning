                ...
                <div class="card-body" style="overflow-x: auto">
                  ...
                  >
                    ...
                    <template #cell(actions)="row">
                      ...
                      <span @click="deleteDocument(row.item.id)">
                        <i
...
export default {
  ...
  methods: {
    ...mapActions("document", ["getDocuments", "delete"]),
    deleteDocument(id) {
      this.$swal({
    ...
    selectAll(checked) {
      this.getDocuments();
      this.selected = checked ? this.documents.map((item) => item.id) : [];
      this.json_data = checked
        ? this.documents.map((item, index) => {
            return {
              STT: index + 1,
              "Tên tài liệu": item.name,
    ...
    selectOne(checked) {
      ...
      this.allSelected = this.documents.length === this.selected.length;
    },
    deleteMutiple() {
      this.$swal({
        title: "Bạn chắc chắn muốn xóa?",
        ...
        if (result.isConfirmed) {
          await this.$store.dispatch("document/deleteMutiple", this.selected);
          this.$swal({
            title: "Đã xóa!",
            icon: "success",
            position: "top-end",
    ...
    exportPdf() {
      ...
      pdfMake.createPdf(docDefinition).download("Danh sách tài liệu.pdf");
    },
  },
  computed: {
