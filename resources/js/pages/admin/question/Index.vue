                  ...
                  >
                    ...
                    <template #cell(actions)="row">
                      <router-link
                        ...
                        ></i
                      ></router-link>
                      <span @click="deleteQuestion(row.item.id)">
                        <i
...
export default {
  ...
  methods: {
    ...mapActions("question", ["getQuestions", "delete"]),
    deleteCLASS(id) {
      this.$swal({
    ...
    selectAll(checked) {
      this.getQuestions();
      this.selected = checked ? this.questions.map((item) => item.id) : [];
      this.json_data = checked
        ? this.questions.map((item, index) => {
            return {
              STT: index + 1,
              "Câu hỏi": item.name,
              "Tên đề thi": item.test.name,
    ...
    selectOne(checked) {
      ...
      this.allSelected = this.questions.length === this.selected.length;
    },
    deleteMutiple() {
      this.$swal({
        title: "Bạn chắc chắn muốn xóa?",
    ...
    exportPdf() {
      ...
      pdfMake.createPdf(docDefinition).download("Danh sách câu hỏi.pdf");
    },
  },
  computed: {
