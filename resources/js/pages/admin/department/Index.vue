                  ...
                  >
                    ...
                    <template #cell(actions)="row">
                      <router-link
                        :to="{
                          name: 'class.add',
                          params: { dept_id: row.item.id },
                        }"
                        ><i
                          class="fas fa-plus fa-lg"
                          v-b-tooltip.hover.v-secondary="'Thêm lớp'"
                        ></i
                      ></router-link>
                      ...
                      <router-link
                        ...
                        ></i
                      ></router-link>
                      <span @click="deleteDepartment(row.item.id)">
                        <i
...
export default {
  ...
  methods: {
    ...
    selectOne(checked) {
      if (checked) {
        this.json_data = this.departments
        ...
        this.pdf_data = this.departments
          .filter((item) => this.selected.includes(item.id))
          .map((item, index) => {
            return [
              index + 1,
      ...
      this.allSelected = this.departments.length === this.selected.length;
    },
    deleteMutiple() {
      this.$swal({
        title: "Bạn chắc chắn muốn xóa?",
