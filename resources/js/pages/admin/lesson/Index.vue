                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <div>
                                                <div v-if="CLASS.teacher && CLASS.teacher.length > 0">
                                                    ...
                                                    <button class="btn btn-primary" @click="getTeacher()">Sửa giảng
                                                        viên</button>
                                                </div>
                                                <div v-else>
                                                    <p>Chưa có giảng viên</p>
                                                    <button class="btn btn-primary" @click="getTeacher()">Thêm giảng
                                                        viên</button>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <b-col md="10" class="my-1">
                                            <div v-for="item in weeks" :key="item.week">
                                                <b-list-group class="mb-5">
                                                    Tuần {{ item.week }}
                                                    <b-list-group-item @click="getSlide(item.week)" href="#"><i
                                                            class="fas fa-graduation-cap fa-lg text-success"></i>Slide
                                                        bài giảng
                                                        {{ item.week }}
                                                        <b-button @click.stop="getCurrentLesson(item.week)"
                                                            v-b-modal.modal-slide
                                                            class="float-right p-0 pl-1 btn-warning btn-sm"><i
                                                                class="fas fa-edit"></i></b-button>
                                                    </b-list-group-item>

                                                    <b-list-group-item href="#" @click="getVideo(item.week)"><i
                                                            class="fas fa-video fa-lg text-danger"></i>
                                                        Video bài giảng
                                                        {{ item.week }}
                                                        <b-button @click.stop="getCurrentLesson(item.week)"
                                                    ...
                                                    <b-list-group-item href="#"><i class="
                                fas
...
getCurrentQuestions(item.week);
                                    ...
                                    <b-modal id="modal-slide" size="lg" :title="'Slide Bài Giảng Tuần ' + lesson.week"
                                        ref="modalSlide">
                                        <b-form-group label="Tên bài giảng" label-for="name-input">
                                            <b-form-input id="name-input" v-model="lesson.name" required :class="{
                                                'is-invalid': errors.name,
                                            }"></b-form-input>
                                            <span v-if="errors.name" id="exampleInputEmail1-error"
                                                class="error invalid-feedback">{{ errors.name[0] }}</span>
                                        </b-form-group>
                                        <b-form-group label="Nội dung" label-for="slide-input">
                                            <b-form-file v-model="lesson.path" id="slide-input" plain :class="{
                                                'is-invalid': errors.path,
                                            }"></b-form-file>
                                            <span v-if="errors.path" id="exampleInputEmail1-error"
                                                class="error invalid-feedback">{{ errors.path[0] }}</span>
                                        </b-form-group>
                                    ...
                                    <b-modal id="modal-video" size="lg" :title="'Video Bài Giảng Tuần ' + lesson.week"
                                        ref="modalVideo">
                                        <b-form-group label="Nội dung" label-for="slide-input">
                                            <b-form-input v-model="lesson.video_path" required :class="{
                                                'is-invalid': errors.video_path,
                                            }"></b-form-input>
                                            <span v-if="errors.video_path" id="exampleInputEmail1-error"
                                                class="error invalid-feedback">{{ errors.video_path[0] }}</span>
                                        </b-form-group>
                                    ...
                                    <b-modal id="modal-view-video" size="lg"
                                        :title="'Video Bài Giảng Tuần ' + lesson.week" ref="modalViewVideo">
                                        <b-form-group>
                                            <div class="d-flex justify-content-center"
                                                v-if="this.lesson.video_path != null">
                                    ...
                                    <b-modal id="modal-test" size="lg" :title="'Bài Luyện Tập Tuần ' + lesson.week"
                                        ref="modalTest">
                                        <b-form-group label="Nhập số câu hỏi" label-for="name-input">
                                            <div class="row">
                                                <div class="col-md-10">
                                                    <b-form-input id="name-input" type="number"
                                                        v-model="number_question" required min="1" :class="{
                                                            'is-invalid': errors.name,
                                                        }"></b-form-input>
                                                </div>
                                                <div class="col-md-2">
                                        ...
                                        <div v-if="questions.length > 0">
                                            <div v-for="(question, index) in questions" :key="question.id">
                                                <b-form-group>
                                                    ...
                                                    <b-form-input id="question" v-model="question.question" required>
                                                    </b-form-input>
                                                </b-form-group>
                                                <b-form-group label="Đáp án" label-for="name-input">
                                                    <div class="row mb-3">
                                                        <div class="col-md-6">
                                                            <b-form-radio v-model="question.correct_Answer"
                                                                ...
                                                                <b-form-input id="answer_A" v-model="question.answer_A"
                                                                    required></b-form-input>
                                                            </b-form-radio>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <b-form-radio v-model="question.correct_Answer"
                                                                :aria-describedby="ariaDescribedby" name="answer"
                                                                value="B">
                                                                <label for="answer_B">Đáp án B</label>
                                                                <b-form-input id="answer_B" v-model="question.answer_B"
                                                                    required></b-form-input>
                                                            </b-form-radio>
                                                        </div>
                                                    </div>
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <b-form-radio v-model="question.correct_Answer"
                                                                :aria-describedby="ariaDescribedby" name="answer_C"
                                                                value="C">
                                                                <label for="answer_C">Đáp án C</label>
                                                                <b-form-input id="answer_C" v-model="question.answer_C"
                                                                    required></b-form-input>
                                                            </b-form-radio>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <b-form-radio v-model="question.correct_Answer"
                                                                :aria-describedby="ariaDescribedby" name="answer_D"
                                                                value="D">
                                                                <label for="answer_D">Đáp án D</label>
                                                                <b-form-input id="answer_D" v-model="question.answer_D"
                                                                    required></b-form-input>
                                                            </b-form-radio>
                                                        </div>
                                                    </div>
                                    ...
                                    <b-modal id="modal-teacher" size="lg" :title="'Thêm giảng viên'" ref="modalTeacher">
                                        <b-form-group label="Nội dung" label-for="slide-input">
                                            <b-form-select v-model="teacher_id" :options="teachers" text-field="name"
                                                value-field="id" :class="{ 'is-invalid': errors.teacher_id }">
                                                <template #first>
                                                    <b-form-select-option :value="null" disabled>-- Chọn giảng viên --
                                                    </b-form-select-option>
                                                </template>
                                            </b-form-select>
                                            <span v-if="errors.video_path" id="exampleInputEmail1-error"
                                                class="error invalid-feedback">{{ errors.teacher_id[0] }}</span>
                                        </b-form-group>
