...
export default {
  ...
  methods: {
    ...mapActions("test", ["getTests", "delete"]),
    deleteTest(id) {
      this.$swal({
    ...
    selectAll(checked) {
      this.getTests();
      this.selected = checked ? this.tests.map((item) => item.id) : [];
      this.json_data = checked
        ? this.tests.map((item, index) => {
            return {
              STT: index + 1,
              "Tên đề thi": item.name,
              Loại: item.type,
    ...
    selectOne(checked) {
      ...
      this.allSelected = this.tests.length === this.selected.length;
    },
    deleteMutiple() {
      this.$swal({
        title: "Bạn chắc chắn muốn xóa?",
        ...
        if (result.isConfirmed) {
          await this.$store.dispatch("test/deleteMutiple", this.selected);
          this.$swal({
            title: "Đã xóa!",
            icon: "success",
            position: "top-end",
    ...
    exportPdf() {
      ...
      pdfMake.createPdf(docDefinition).download("Danh sách đề thi.pdf");
    },
  },
  computed: {
